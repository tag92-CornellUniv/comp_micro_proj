...
```{r load-packages}
pacman::p_load(phytools, ggtree, RColorBrewer, tidyverse, phyloseq, microViz, install = FALSE)
install.packages(
  "microViz",
...
## Examine NA classification sequences - evaluate lengths before filtering

```{r evaluate-seq-lengths}

## physeq object - clean_physeq

taxtable <-
  clean_physeq@tax_table %>%
  data.frame() %>%
  mutate(ASVlength = nchar(ASVSeq))
...
### CHECK IF MOST DOMINANT ASV IS REVERSE COMPLEMENT 
```
...
There is still a crazy separation happening, even after removing the unclassified sequences.
...
Filtering vs not filtering the tree both result in a weird, split tree. Almost all sequences on both halves of the rooted tree are the exact same length, so I am not really sure what is happening here or what to make of this data!!

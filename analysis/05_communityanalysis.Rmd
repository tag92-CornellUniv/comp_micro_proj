...
combined_groups <- interaction(sample_data(scaled_unfiltered_rooted_physeq)$burn_timeframe, sample_data(scaled_unfiltered_rooted_physeq)$horizon)
...
plot_ordination(
  physeq = scaled_unfiltered_rooted_physeq,
  ordination = unfiltered_unifrac,
  color = combined_groups,  # Use combined variable for coloring
  title = "Unfiltered-Kingdom Dataset Weighted Unifrac PCoA"
) +
  scale_color_gradient(values=combined_horizon_timeframe) +
geom_point(size = 4, alpha = 0.8, aes(color = combined_groups))
...
SORENSENPLOT + BRAYPLOT + WUNIFRAC
...
## Check filtered read counts

```{r filtered-read-counts}

```

## Normalize filtered read counts
```{r normalize-filtered-read-counts}

```

## Filtered community analyses

### Sorensen 

```{r filtered-dataset-sorensen}

```

### Bray-Curtis
```{r filtered-dataset-braycurtis}

```

### Weighted Unifrac
```{r filtered-weighted-unifrac}

```

### Permanova 

```{r filtered-permanova}

...
#Create the vectors for both forward and reverse reads
...
##Check the quality of forward and reverse reads
```{r quality-check}
...
##Placeholders for filtered reads
```{r placeholders}
...
#very stringent, limited errors and mismatches, removed primers from reads
```

##Look at the quality of trimmed reads
...
##Stats for read counts of samples
...
#plot both the forward and reverse reads
...
## Merge the f and r reads into contigs
...
## Evaluate remaining reads
```{r evaluate-remaining-reads}
...
## Assign taxonomy to the remaining reads
```{r Assign-taxonomy-to-the-remaining-reads}
taxa_train <- 
...
##Generate the final ASV table for export
```{r final-asv-table}
...
##Save DADA2 outputs
```{r save-DADA2-outputs}
write.table(asv_tab, "./DADA2_amplicon-proj_runthrough_files/output_DADA2_ASV_counts.tsv", sep = "\t", quote = FALSE, col.names = NA)
write.table(noChimeras_ASV_table, "./DADA2_amplicon-proj_runthrough_files/output_DADA2_ASV_counts_withSeqNames.tsv", sep = "\t", quote = FALSE, col.names = NA)
asv_fasta <- c(rbind(asv_headers, asv_seqs))
write(asv_fasta, "./DADA2_amplicon-proj_runthrough_files/output_DADA2_ASVs.fasta")


# Write the table 
write.table(asv_tax, "./DADA2_amplicon-proj_runthrough_files/output_DADA2_ASV_taxonomy.tsv", sep = "\t", quote = FALSE, col.names = NA)

# RData objects are for easy loading :) 
save(noChimeras_ASV_table, file = "./DADA2_amplicon-proj_runthrough_files/output_DADA2_noChimeras_ASV_table.RData")
save(asv_tab, file = "./DADA2_amplicon-proj_runthrough_files/output_DADA2_ASV_counts.RData")

save(track_counts_df, file = "./DADA2_amplicon-proj_runthrough_files/output_DADA2_track_read_counts.RData")
